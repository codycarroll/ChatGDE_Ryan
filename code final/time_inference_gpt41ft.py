"""
Time pure inference (no embedding computation) for GPT-4.1 FT on 3 GSPs
with pre-cached embeddings.

Run from repo root:
    python3 "code final/time_inference_gpt41ft.py"
"""
...
for gid, gsp_name in TIMING_GSPS:
    ...
    n_pages = len(pages)
    print(f'\n[{gid}] {gsp_name} ({n_pages} pages)', flush=True)
    ...
    print(f'  [{gsp_name}] done — {gsp_elapsed:.2f} min', flush=True)
