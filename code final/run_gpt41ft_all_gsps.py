...
Retrieval: page-level cosine similarity.
Pages are pre-extracted text files in PAGES_DIR.
Embeddings are computed on first run and cached in EMB_DIR.

Output (run from repo root: python3 "code final/run_gpt41ft_all_gsps.py"):
    results/checkpoint_gpt41ft_allgsps_<timestamp>.json
    results/results_gpt41ft_allgsps_<timestamp>.csv

Cost test:
    python3 "code final/run_gpt41ft_all_gsps.py" --test
"""
...
def load_or_compute_embeddings(gsp_name, pages_path):
    ...
    print(f'  Computing embeddings for {gsp_name} ({len(pages)} pages)...', flush=True)
    embeddings = [get_embedding(p) for p in pages]
    with open(emb_path, 'w') as f:
        json.dump(embeddings, f)
    print(f'  Cached embeddings to {emb_path}', flush=True)
    return pages, embeddings
...
# ── Discover GSPs ─────────────────────────────────────────────────────────────
print('Discovering GSPs...', flush=True)
...
# ── Evaluation loop ───────────────────────────────────────────────────────────
if args.test:
    print('*** TEST MODE — stopping after first GSP ***', flush=True)

print(f'\n{"="*60}', flush=True)
print(f'GPT-4.1 FT — {len(gsp_ids)} GSPs × {N_QUESTIONS} questions', flush=True)
...
for gid in gsp_ids:
    ...
    try:
        ...
        print(f'  Failed to load embeddings: {e} — skipping', flush=True)
        continue
    ...
    gsps_run += 1
    if args.test:
        print(f'\nTest complete — check platform.openai.com/usage for cost.', flush=True)
        break
...
for gid in gsp_ids:
    ...
    if len(human) != len(probs):
        print(f'  ⚠ {gsp_name}: rubric {len(human)} rows vs {len(probs)} responses '
              f'— skipping from CSV', flush=True)
        continue
...
print(f'GSPs scored: {df["GSP_ID"].nunique()}', flush=True)
print(f'Saved: {csv_path}', flush=True)
for g, a in df_valid.groupby('GSP')['correct'].mean().items():
    print(f'  {g:<40s} {a:.1%}', flush=True)
print(f'{"="*60}', flush=True)
