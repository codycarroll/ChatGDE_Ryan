"""Usage: python _run_vision_gsp.py <gsp_id> <cache_name> <display_name> <pdf_path> <rubric_file> <sonnet_display_name>"""
import sys, re, os, pickle, base64, json
...
with open(os.path.expanduser('~/Desktop/chatgde_api'), 'r') as f:
    openai_key = re.search(r'sk-[A-Za-z0-9_-]+', f.read()).group(0)
with open(os.path.expanduser('~/Desktop/anthropic_api'), 'r') as f:
    anthropic_key = re.search(r'sk-ant-[A-Za-z0-9_-]+', f.read()).group(0)
...
if os.path.exists(CACHE_PATH):
    ...
    print(f'[{display_name}] Extracting chunks...', end=' ', flush=True)
    cwp = extract_chunks_with_pages(pdf_path)
    print(f'{len(cwp)} chunks. Computing embeddings...', end=' ', flush=True)
