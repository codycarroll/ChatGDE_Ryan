...
Uses same retrieval pipeline + system prompt as all other models.

Run from repo root:
    python3 "code final/run_gpt55_base_eval.py"
"""
...
print(f'\n{"="*60}', flush=True)
print(f'GPT-5.5 Base — {N_QUESTIONS} questions × 5 GSPs = {N_QUESTIONS * 5} total', flush=True)
...
print(f'\n{"="*55}', flush=True)
print(f'GPT-5.5 Base — Overall: {overall:.1%}  ({df["correct"].sum()}/{len(df)})', flush=True)
