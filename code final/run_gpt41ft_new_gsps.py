"""
Evaluate GPT-4.1 FT v4 on 6 new GSPs not included in the original all-GSP run.

New GSPs:
  4  SouthAmerican
  8  LosMolinos
  48 PleasantValley
  57 SantaYnezRiverValleyWestern  (previously SKIP_ID — new draft PDF)
  58 SantaYnezRiverValleyCentral  (previously SKIP_ID — new draft PDF)
  59 SantaYnezRiverValleyEastern  (new draft PDF supersedes old pages file)

Pages are extracted inline from PDFs in GSP_Drafts/ using pdfplumber.
Embeddings are cached in results/gsp_embeddings/.

Run from repo root:
  python3 "code final/run_gpt41ft_new_gsps.py"

Outputs:
  results/checkpoint_gpt41ft_newgsps_<timestamp>.json
  results/results_gpt41ft_newgsps_<timestamp>.csv
  (gpt41ft_allgsps_per_gsp_metrics.csv updated in-place with new rows)
"""
import re, os, json, time, glob, argparse
...
MODEL      = 'ft:gpt-4.1-2025-04-14:personal:gspv4:DbO4oSN8'
MODEL_NAME = 'gpt41ft'
...
def load_or_compute_embeddings(gsp_name, pages):
    ...
    if os.path.exists(emb_path):
        ...
        print(f'  Loaded cached embeddings ({len(embeddings)} pages)', flush=True)
        return embeddings
    print(f'  Computing embeddings ({len(pages)} pages)...', flush=True)
    embeddings = [get_embedding(p) for p in pages]
    with open(emb_path, 'w') as f:
        json.dump(embeddings, f)
    print(f'  Cached: {emb_path}', flush=True)
    return embeddings
...
# ── Evaluation loop ───────────────────────────────────────────────────────────
print(f'\n{"="*60}', flush=True)
print(f'GPT-4.1 FT — {len(NEW_GSPS)} new GSPs × {N_QUESTIONS} questions', flush=True)
...
for gid, gsp_name, pdf_file, rubric_file in NEW_GSPS:
    ...
    if not os.path.exists(pdf_path):
        print(f'[{gid:2d}] {gsp_name} — PDF not found: {pdf_path}', flush=True)
        continue
    if not os.path.exists(rubric_path):
        print(f'[{gid:2d}] {gsp_name} — Rubric not found: {rubric_path}', flush=True)
        continue
    ...
    if start_idx == N_QUESTIONS:
        print(f'[{gid:2d}] {gsp_name} — already complete', flush=True)
        continue

    print(f'\n[{gid:2d}] {gsp_name} — {start_idx}/{N_QUESTIONS} done', flush=True)

    # Extract pages from PDF
    print(f'  Extracting pages from PDF...', flush=True)
    ...
    for idx, qi in enumerate(active_qs[start_idx:], start=start_idx):
        ...
        if answered % 10 == 0 or answered == N_QUESTIONS:
            print(f'  Q{qi:2d} ({answered}/{N_QUESTIONS}): {parse_answer(resp)}', flush=True)

    print(f'  [{gsp_name}] done.', flush=True)

    if args.test:
        print('\nTest mode — stopping after first GSP.', flush=True)
        break

# ── Build results CSV ─────────────────────────────────────────────────────────
print(f'\nBuilding results CSV...', flush=True)
...
for gid, gsp_name, _, rubric_file in NEW_GSPS:
    ...
    if len(human) != len(probs):
        print(f'  ⚠ {gsp_name}: rubric {len(human)} rows vs {len(probs)} responses — skipping',
              flush=True)
        continue
...
# ── Per-GSP metrics ───────────────────────────────────────────────────────────
print(f'\nComputing per-GSP metrics...', flush=True)
...
print(f'\nUpdated: {METRICS_CSV}  ({len(df_metrics)} GSPs total)', flush=True)
...
print(f'New GSPs — Binary accuracy: {overall:.1%}  '
      f'({df_valid["correct"].sum()}/{len(df_valid)})', flush=True)
